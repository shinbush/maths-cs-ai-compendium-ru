# Примеры проектирования машинного обучения

*Лучший способ изучить проектирование систем машинного обучения — это проработанные примеры. В этом файле рассматриваются семь полных проектов: системы рекомендаций, рейтинг поиска, прогнозирование кликов по рекламе, обнаружение мошенничества, модерация контента, диалоговый искусственный интеллект и крупномасштабный поиск изображений.*

- Каждый пример следует последовательной схеме:
    1. **Формирование проблемы**: что мы создаем, кто пользователи, каковы ограничения?
    2. **Данные**: какие данные у нас есть, как они собираются и как маркируются?
    3. **Особенности**: какие функции необходимы модели?
    4. **Модель**: какая архитектура и подход к обучению?
    5. **Обслуживание**: как развертывается и обслуживается модель?
    6. **Оценка**: как измерить успех?
    7. **Итерация**: какие улучшения мы можем внести со временем?

---

## 1. Система рекомендаций (например, YouTube, Netflix, Spotify)

### Формулировка проблемы

– **Цель**: показывать пользователям контент, который им понравится, максимально повышая вовлеченность (время просмотра, прослушивания, клики).
- **Масштаб**: более 1 миллиарда пользователей, более 100 миллионов элементов, более 10 тысяч рекомендаций в секунду.
- **Задержка**: <200ms for the full recommendation pipeline.
...
```
Пользовательский запрос → Встраивание запроса → Векторный поиск (документация) → Top-K фрагменты
                                                                      ↓
Пользовательский запрос + Полученные фрагменты → LLM → Ответ (с цитатами)
```
...
```
Система: Вы полезный помощник. Ответ основан ТОЛЬКО на предоставленном контексте.
Если ответ не в контексте, скажите: «Я не знаю».
...
```
Изображение запроса → Модель внедрения (ViT/CLIP) → Вектор 512-dim → Поиск ИНС → Результаты Top-K
```

### Встраивание извлечения

- **Модель**: предварительно обученный видеокодер (ViT, кодировщик изображений CLIP, DINOv2). При необходимости выполняется точная настройка для конкретной области (мода, электронная коммерция, медицинская визуализация).

- **Обучение**: контрастное обучение (глава 10). Положительные пары = разные виды одного и того же изображения (или изображения + совпадающего текста). Отрицательные пары = случайные изображения. Модель учится создавать похожие вложения для похожих изображений и разные вложения для разных изображений.

### Индексирование

- **Офлайн**: вставьте все изображения 1B и создайте индекс ANN. Для HNSW (файл 03) построение индекса занимает несколько часов, и индекс хранится в памяти (~ 128 ГБ для 1B × 512-dim × float16 + издержки графа).

- **Шардинг**: разделение индекса на несколько компьютеров. Каждая машина содержит осколок. Во время запроса выполните параллельный поиск по всем сегментам и объедините первые K результатов.

- **Дополнительные обновления**: в индекс необходимо добавлять новые изображения (загрузки, новые продукты). HNSW поддерживает инкрементальную вставку без перестроения. Векторные базы данных (Milvus, Pinecone) справляются с этой задачей изначально.

### Обслуживание

- **Служба внедрения**: сервер графического процессора, на котором работает модель ViT. Задержка: ~20 мс на изображение. Пакетная обработка нескольких запросов для определения пропускной способности.

- **Служба поиска**: индексный сервер ANN. Задержка: ~ 10 мс для поиска в топ-100 по 1 млрд векторов (с HNSW).

- **Кэширование**: кэшируйте результаты по популярным запросам. Для обнаружения дубликатов кэшируйте встраивание недавно загруженных изображений и сравнивайте новые загрузки с кешем перед поиском по полному индексу.

### Оценка

- **Precision@K**: действительно ли результаты топ-К схожи?
- **Recall@K**: сколько из действительно похожих изображений в корпусе находится в топ-К?
- **Средняя средняя точность (mAP)**: область под кривой точности-воспроизведения.
- **Человеческая оценка**: на предмет субъективного сходства оценщики оценивают, являются ли полученные изображения релевантными.

---

## Структура интервью

- Когда вы сталкиваетесь с вопросом проектирования системы, следуйте следующей схеме:

1. **Уточнение требований** (2–3 минуты): спросите о масштабе, задержке, требованиях к согласованности и крайних случаях. «Сколько пользователей? Какая задержка приемлема? Что происходит во время сбоев?»

2. **Проектирование высокого уровня** (5–7 минут): нарисуйте основные компоненты и их взаимодействие. Начните со счастливого пути. Используйте выкройки из файлов 01-03.3. **Глубокое погружение** (15–20 минут): выберите самый интересный/сложный компонент и детально спроектируйте его. Здесь вы показываете глубину. Для системы машинного обучения часто требуется глубокое погружение в архитектуру модели, конвейер функций или архитектуру обслуживания.

4. **Оценка и мониторинг** (3–5 минут): как вы оцениваете успех? Что может пойти не так? Как вы обнаруживаете проблемы и реагируете на них?

5. **Итерация** (2–3 минуты): что бы вы улучшили, потратив больше времени/ресурсов? Это покажет, что вы понимаете компромиссы и умеете расставлять приоритеты.

- **Чего ищут интервьюеры**: структурированное мышление (не спешить с решениями), понимание компромиссов (каждый выбор имеет свою цену), практические знания (вы действительно создали системы) и коммуникацию (можете ли вы четко объяснить свой дизайн?).