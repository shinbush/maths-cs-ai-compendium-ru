...
def best_split(X, y):
    ...
    вернуть best_feat, best_thresh, best_ig

# Пример: синтетические данные
из sklearn.datasets импорт make_classification
X, y = make_classification(n_samples=100, n_features=4, n_redundant=0, случайное_состояние=0)
X, y = jnp.array(X), jnp.array(y)

подвиг, треш, ig = best_split(X, y)
print(f"Лучшее разделение: функция {feat}, порог {thresh:.3f}, прирост информации {ig:.4f}")
print(f"Родительский Джини: {gini_impurity(y):.4f}")
маска = X[:, feat] <= треш
print(f"Левый Джини: {gini_impurity(y[mask]):.4f} ({int(jnp.sum(mask))} образцы)")
print(f"Правильный Джини: {gini_impurity(y[~mask]):.4f} ({int(jnp.sum(~mask))} образцы)")
```
...
из sklearn.datasets импортируйте make_blobs

# Генерация синтетических кластеров
X, y_true = make_blobs(n_samples=300,centers=4,cluster_std=0,8,random_state=42)
X = jnp.array(X)

def kmeans_plus_plus_init (X, K, ключ):
    """Инициализация K-Means++."""
    n = X.форма[0]
    idx = jax.random.randint(ключ, (), 0, n)
    центроиды = [X[idx]]
    для _ в диапазоне (1, K):
        dists = jnp.min(jnp.stack([jnp.sum((X - c)**2, axis=1) для c в центроидах]), axis=0)
        пробы = dists/jnp.sum(dists)
        ключ, подраздел = jax.random.split(ключ)
        idx = jax.random.choice(подключ, n, p=probs)
        centroids.append(X[idx])
    вернуть jnp.stack(центроиды)

def kmeans(X, K, max_iters=20, key=jax.random.PRNGKey(0)):
    центроиды = kmeans_plus_plus_init (X, K, ключ)
    история = [центроиды]
    для _ в диапазоне (max_iters):
        # Назначить шаг
        dists = jnp.stack([jnp.sum((X - c)**2, axis=1) для c в центроидах])
        labels = jnp.argmin(dists, axis=0)
        # Шаг обновления
        new_centroids = jnp.stack([
            jnp.mean(X[labels == k], axis=0) для k в диапазоне (K)
        ])
        History.append(new_centroids)
        если jnp.allclose(centroids, new_centroids):
            сломать
        центроиды = новые_центроиды
    метки возврата, центроиды, история

К = 4
метки, центроиды, история = kmeans(X, K)

# Постройте окончательный результат
цвета = ['#3498db', '#e74c3c', '#27ae60', '#9b59b6']
plt.figure(figsize=(8, 6))
для k в диапазоне (K):
    маска = метки == k
    plt.scatter(X[маска, 0], X[маска, 1], c=colors[k], s=20, альфа=0,6)
    plt.scatter(centroids[k, 0], centroids[k, 1], c=colors[k], маркер='X',
                s=200, EdgeColors='k', Linewidths=1.5)
plt.title(f"Кластеризация K-средних (K={K}, {len(history)-1} итераций)")
plt.grid(альфа=0,3)
plt.show()

# Вычисление инерции
инерция = sum(jnp.sum((X[labels == k] - центроиды[k])**2) для k в диапазоне(K))
print(f"Конечная инерция: {inertia:.2f}")
```
...
# Полиномиальное ядро: K(x,y) = (x·y + 1)^2
def poly_kernel(X, степень=2, c=1.0):
    возврат (X @ X.T + c) ** степень

# Явная карта признаков степени 2 для 2D: (1, sqrt(2)*x1, sqrt(2)*x2, x1^2, x2^2, sqrt(2)*x1*x2)
защита Poly_features (X):
    x1, x2 = X[:, 0], X[:, 1]
    вернуть jnp.column_stack([
        jnp.ones(len(X)),
        jnp.sqrt(2) * x1,
        jnp.sqrt(2) * x2,
        х1**2,
...
print("Трюк ядра (полиномиальная степень 2):")
печать (K_trick)
print("\nСкалярное произведение карты явных признаков:")
печать (K_explicit)
print(f"\nСоответствие матриц: {jnp.allclose(K_trick, K_explicit)}")# Ядро RBF: конечного явного отображения не существует
def rbf_kernel(X, гамма=0,5):
    sq_dists = jnp.sum(X**2, axis=1, Keepdims=True) + \
               jnp.sum(X**2, ось=1) - 2 * X @ X.T
    return jnp.exp(-gamma * sq_dists)

K_rbf = rbf_kernel(X)
print("\nМатрица ядра RBF:")
печать (K_rbf)
print("Диагональ всегда равна 1 (точка идентична самой себе)")
print("Недиагональные записи затухают с расстоянием")
```